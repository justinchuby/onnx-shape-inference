...
class SymbolicShapeInference:
    ...
    def _add_suggested_merge(self, symbols, apply=False):
        ...
        if map_to is None:
            if self.verbose_ > 0:
                logging.warning(
                    f"Potential unsafe merge between symbolic expressions: ({','.join(symbols)})"
                )
    ...
    def _preprocess(self, in_mp):
        ...
        self.known_vi_.update(
            {
                i.name: onnx.helper.make_tensor_value_info(
    ...
    def _get_value(self, node, idx):
        ...
        return (
            ...
            else onnx.numpy_helper.to_array(self.initializers_[name])
        )
    ...
    def _onnx_infer_single_node(self, node):
        ...
        if not skip_infer:
            ...
            tmp_graph = onnx.helper.make_graph(
                [node],
                "tmp",
            ...
            self.tmp_mp_ = onnx.shape_inference.infer_shapes(self.tmp_mp_)
    ...
    def _compute_matmul_shape(self, node, output_dtype=None):
        ...
        vi = self.known_vi_[node.output[0]]
        vi.CopyFrom(
            onnx.helper.make_tensor_value_info(node.output[0], output_dtype, new_shape)
        )
    ...
    def _fuse_tensor_type(self, node, out_idx, dst_type, src_type):
        ...
        if dst_tensor_type.HasField("shape"):
            for di, ds in enumerate(
                ...
                if ds[0] != ds[1]:
                    # create a new symbolic dimension for node/out_idx/mismatch dim id in
                    # dst_tensor_type for tensor_type
    ...
    def _infer_symbolic_compute_ops(self, node):
        funcs = {
            ...
            "Mul": lambda l: int(l[0] * l[1])  # noqa: E741
            if isinstance(l[0] * l[1], float)
            else l[0] * l[1],
    ...
    def _infer_Constant(self, node):  # noqa: N802
        ...
        self.sympy_data_[node.output[0]] = onnx.numpy_helper.to_array(t)
    ...
    def _infer_ConstantOfShape(self, node):  # noqa: N802
        ...
        if sympy_shape is not None:
            ...
            if vi.type.tensor_type.elem_type == onnx.TensorProto.INT64 and all(
                ...
                ) * onnx.numpy_helper.to_array(get_attribute(node, "value", 0))
        else:
    ...
    def _infer_Einsum(self, node):  # noqa: N802
        ...
        if mid_index != -1:
            ...
            for c in right_equation:
                # FIXME(justinchuby): Improve this code
                if c != 46:  # c != b'.'
        ...
        vi.CopyFrom(
            onnx.helper.make_tensor_value_info(
                node.output[0], output_dtype, new_sympy_shape
            )
        )
    ...
    def _infer_Gather(self, node):  # noqa: N802
        ...
        if (
            ...
            and get_attribute(node, "axis", 0) == 0
        ):
    ...
    def _infer_aten_upsample(self, node):
        ...
        if input_shape is not None:
            ...
            if output_size is not None:
                new_shape += [
                    dim_size.item() if type(dim_size) == np.int64 else dim_size
                    for dim_size in output_size
                ]
            else:
    ...
    def _infer_Scan(self, node):  # noqa: N802
        ...
        # We may have cases where the subgraph has optional inputs that appear in both
        # subgraph's input and initializer, but not in the node's input.
        # In such cases, the input model might be invalid, but let's skip those optional inputs.
        assert len(subgraph.input) >= len(node.input)
    ...
    def _infer_Slice(self, node):  # noqa: N802
        ...
        def flatten_min(expr):
            assert isinstance(
                expr, sympy.Add
            ), f"Expected a sum of two arguments, got {expr}"
            min_positions = [
                idx
        ...
        def handle_negative_index(index, bound):
            ...
            try:
                ...
                logging.warning(f"Cannot determine if {index} < 0")
            return index
    ...
    def _infer_SoftmaxCrossEntropyLoss(self, node):  # noqa: N802
        ...
        if len(node.output) > 1:
            ...
            vi.CopyFrom(
                onnx.helper.make_tensor_value_info(vi.name, elem_type, data_shape)
            )
    ...
    def _infer_Split(self, node):  # noqa: N802
        self._infer_Split_Common(node, onnx.helper.make_tensor_value_info)

    def _infer_SplitToSequence(self, node):  # noqa: N802
        self._infer_Split_Common(node, onnx.helper.make_sequence_value_info)
    ...
    def _infer_TopK(self, node):  # noqa: N802
        ...
        if type(k) in [int, str]:
            ...
            # note that TopK dim could be computed in sympy_data, so need to update
            # computed_dims when it enters shape
            self._update_computed_dims(new_sympy_shape)
    ...
    def _infer_Attention(self, node):  # noqa: N802
        ...
        tripled_hidden_size = (
            shape_bias[0] if shape_bias is not None else shape_weights[1]
        )
        if shape and len(shape) == 3:
            ...
            vi = self.known_vi_[node.output[0]]
            vi.CopyFrom(
                onnx.helper.make_tensor_value_info(node.output[0], output_dtype, shape)
            )
            ...
            if len(node.output) > 1:
                ...
                input_shape = self._get_shape(node, 0)
                past_shape = (
                    self._get_shape(node, 4)
                    if len(node.input) > 4 and node.input[4]
                    else []
                )
                mask_shape = (
                    self._get_shape(node, 3)
                ...
                if past_shape and len(past_shape) == 5:
                    if mask_shape and len(mask_shape) in [2, 3]:
                        ...
                        if isinstance(input_shape[1], int) and isinstance(
                            past_shape[3], int
                        ):
                    ...
                    vi.CopyFrom(
                        onnx.helper.make_tensor_value_info(
                            vi.name, output_dtype, past_shape
                        )
                    )
                # No past input but present output still exists
                    ...
                    vi.CopyFrom(
                        onnx.helper.make_tensor_value_info(
                            vi.name, output_dtype, present_shape
                        )
                    )
    ...
    def _infer_RemovePadding(self, node):  # noqa: N802
        ...
        if shape and len(shape) == 3:
            ...
            vi.CopyFrom(
                onnx.helper.make_tensor_value_info(
                    node.output[0], output_dtype, ["token_count", shape[2]]
                )
            )
            ...
            vi_token_offset.CopyFrom(
                onnx.helper.make_tensor_value_info(
                    node.output[1], onnx.TensorProto.INT32, [shape[0], shape[1]]
                )
            )
            ...
            vi_cumulated_seq_len.CopyFrom(
                onnx.helper.make_tensor_value_info(
                    node.output[2], onnx.TensorProto.INT32, ["batch_size + 1"]
                )
            )

            vi_max_seq_len = self.known_vi_[node.output[3]]
            vi_max_seq_len.CopyFrom(
                onnx.helper.make_tensor_value_info(
                    node.output[3], onnx.TensorProto.INT32, [1]
                )
            )
    ...
    def _infer_RestorePadding(self, node):  # noqa: N802
        ...
        if (
            shape_input
            and len(shape_input) == 2
            and shape_token_offset
            and len(shape_token_offset) == 2
        ):
            output_dtype = self.known_vi_[node.input[0]].type.tensor_type.elem_type
            vi = self.known_vi_[node.output[0]]
        ...
        if query_shape is not None:
            ...
            if len(node.output) > 1:
                ...
                if past_shape is not None:
                    if isinstance(past_shape[2], int) and isinstance(
    ...
    def _infer_LayerNormalization(self, node):  # noqa: N802
        ...
        if len(node.output) > 1:
            ...
            if x_shape is not None:
                ...
                vi.CopyFrom(
                    onnx.helper.make_tensor_value_info(
                        node.output[1], mean_dtype, mean_shape
                    )
                )
                if len(node.output) > 2:
                    ...
                    vi.CopyFrom(
                        onnx.helper.make_tensor_value_info(
                            node.output[2], mean_dtype, mean_shape
                        )
                    )
    ...
    def _infer_BiasSplitGelu(self, node):  # noqa: N802
        ...
        if input_shape and bias_shape and isinstance(bias_shape[0], int):
            ...
            output_dtype = self.known_vi_[node.input[0]].type.tensor_type.elem_type
            vi.CopyFrom(
                onnx.helper.make_tensor_value_info(vi.name, output_dtype, output_shape)
            )
    ...
    def _infer_PythonOp(self, node):  # noqa: N802
        ...
        if get_attribute(node, "name").decode() in [
            ...
            assert (
                len(node.output) == len(node.input) + 1
            )  # The output contains one extra context info.
    ...
    def _infer_impl(self, start_sympy_data=None):
        ...
        if any(o.name in sorted_known_vi for o in self.out_mp_.graph.output):
            ...
            while not all(o.name in sorted_known_vi for o in self.out_mp_.graph.output):
                ...
                if old_sorted_nodes_len == len(sorted_nodes) and not all(
                    ...
                    msg = "Invalid model with cyclic graph"
                    raise Exception(msg)
        ...
        for node in sorted_nodes:
            ...
            if self.verbose_ > 2:
                logging.debug(node.op_type + ": " + node.name)
            ...
            for i_o in range(len(node.output)):
                ...
                if (
                    node.op_type
                    in {"SkipLayerNormalization", "SkipSimplifiedLayerNormalization"}
                ) and i_o in [1, 2]:
                    continue
                    ...
                    if self.verbose_ > 2:
                        if out_type_kind == "sequence_type":
                            ...
                            if seq_cls_type == "tensor_type":
                                logging.debug(
                                    f"  {node.output[i_o]}: sequence of {get_shape_from_value_info(vi)} {onnx.TensorProto.DataType.Name(vi.type.sequence_type.elem_type.tensor_type.elem_type)}"
                                )
                            else:
                                logging.debug(
                                    f"  {node.output[i_o]}: sequence of {seq_cls_type}"
                                )
                        else:
                            logging.debug(f"  {node.output[i_o]}: {out_type_kind}")
                    continue
                ...
                if self.verbose_ > 2:
                    logging.debug(
                        f"  {node.output[i_o]}: {out_shape} {onnx.TensorProto.DataType.Name(vi.type.tensor_type.elem_type)}"
                    )
                ...
                if (
                    ...
                    if self.auto_merge_:
                        ...
                        if shapes:
                            for idx in range(len(out_shape)):
                                ...
                                # if a tensor has a lower rank (dim_idx[idx] < 0), it would automatically broadcast and
                                # need no merge
                                dim_idx = [
                                    len(s) - len(out_shape) + idx for s in shapes
                                ]
                    ...
                    if (
                        ...
                        if out_rank >= 0:
                            ...
                            if self.verbose_ > 0:
                                ...
                                if self.verbose_ > 2:
                                    logging.debug(
                                        f"  {node.output[i_o]}: {new_shape} {vi.type.tensor_type.elem_type}"
                                    )
                    ...
                    if self.verbose_ > 0 or not self.auto_merge_ or out_type_undefined:
                        ...
                        logging.debug("node inputs:")
                        for i in node.input:
                            if i in self.known_vi_:
                                logging.debug(self.known_vi_[i])
                            else:
                                logging.debug(f"not in knwon_vi_ for {i}")
                        logging.debug("node outputs:")
                        for o in node.output:
                            if o in self.known_vi_:
                                logging.debug(self.known_vi_[o])
                            else:
                                logging.debug(f"not in knwon_vi_ for {o}")
                        if self.auto_merge_ and not out_type_undefined:
                            logging.debug("Merging: " + str(self.suggested_merge_))
                    return False
    ...
    def infer_shapes(
        ...
        if not all_shapes_inferred:
            ...
            msg = "Incomplete symbolic shape inference"
            raise Exception(msg)
...
if __name__ == "__main__":
    ...
    logging.info("input model: " + args.input)
    if args.output:
        logging.info("output model " + args.output)
    logging.info("Doing symbolic shape inference...")
